...
function makeEventsState(overrides: Partial<RalphLoopState> = {}): RalphLoopState {
...
test("session_before_switch blocks resume while loop is running", async () => {
  ...
  assert.deepEqual(h.notifications.at(-1), {
    message: "Ralph loop is running. /resume is blocked. Use another pi instance or /ralph-stop.",
    type: "warning",
  });
});
